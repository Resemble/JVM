...
网络层               负责管理网络地址，定位设备，决定路由。IP地址和路由器就是工作在这一层。
                        ...
                        路由信息的交换。                                                      IP、ICMP（Internet Control Message Protocol网际控制信息协议 ping使用）、
                                                                                                RIP(Router Information Protocol 路由信息协议)、OSPF(开放最短路径优先协议)、IGMP(内部网关路由协议)  
...
但`PATCH一般用于资源的部分更新`，而PUT一般用于资源的整体更新。
...
2）匿名管道/命名管道;
3）邮槽。
其中`共享内存、匿名管道只能实现两个进程间的通信`，不能跨网络通信；命名管道和邮槽可以跨网络进程间通信；但命名管道属于点对点的通信，
可传输大数据量；而邮槽一次传输的数据量非常有限，通常少于424字节。
