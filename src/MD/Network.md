...
##### 滑动窗口过大过小
1.滑动窗口过大，也就是在两次ACK之间一次性发送的字段太多，容易造成接收端来缓存不够而溢出。而且当信道质量不好的时候，会造成重传太多，
通信效率严重下降。拥塞
2.滑动窗口过小，极端的情况就是停止等待协议，发一个字段等一个ACK，也会造成通信效率下降
...
#### 快重传
发送端只要一连收到三个重复的 ACK 即可断定有分组丢失了，就应立即重传丢失的报文段而不必继续等待为该报文段设置的重传计时器的超时。Ack2 丢失了 Ack3
#### 快恢复
...
DELETE方法也是幂等的。 HTTP方法的幂等性是指一次和多次请求某一个资源应该具有同样的副作用。
- CONNECT
...
两次相同的POST请求会在服务器端创建两份资源，它们具有不同的URI；所以，POST方法不具备幂等性。而PUT所对应的URI是要创建或更新的资源本身。
...
3、如果hosts与本地DNS解析器缓存都没有相应的网址映射关系，首先会找TCP/ip参数中设置的首选DNS服务器，在此我们叫它本地DNS服务器，此服务器
收到查询时，如果要查询的域名，包含在本地配置区域资源中，则返回解析结果给客户机，完成域名解析，此解析具有权威性。 
4、如果要查询的域名，不由本地DNS服务器区域解析，但该服务器已缓存了此网址映射关系，则调用这个IP地址映射，完成域名解析，此解析不具有权威性。 
5、如果本地DNS服务器本地区域文件与缓存解析都失效，则根据本地DNS服务器的设置（是否设置转发器）进行查询，如果未用转发模式，本地DNS就把
请求发至13台根DNS，根DNS服务器收到请求后会判断这个域名(.com)是谁来授权管理，并会返回一个负责该顶级域名服务器的一个IP。

#### ping流程
利用IP地址的唯一性,给目标IP地址发送一个数据包，再要求对方返回一个同样大小的数据包来确定两台网络机器是否连接相通，时延是多少。
- 在同一个网段内
在主机A上运行"ping 192.168.0.1"之后，ping命令会构建一个固定格式的ICMP（Internet Control Messages Protocol）请求数据包，然后
ICMP协议将这个数据包连同地址"192.168.0.1"，一起交给IP层协议，IP层协议将以地址“192.168.0.1”作为目的地址，本机IP地址作为源地址，
加上一些其他的控制信息，构建一个IP数据包。IP层协议通过同一个网段内的另一台主机B的IP地址和B的子网掩码，发现主机A与B属于同一网络
（将IP地址与子网掩码作与（&）运算，如果结果相同，则在同一个网段内）。接着，就直接在本网络内查找目的IP地址对应的MAC地址。
通过ARP协议，将目的IP地址转换为MAC地址。将IP地址转换为MAC地址的大致的流程是：在主机A的ARP缓存表中查找目的IP地址与主机B的映射关系；
如果没有找到，就发一个ARP请求广播，得到B主机的MAC地址。
得到MAC地址后，附上一些控制信息后将数据交给数据链路层。数据链路层构建一个数据帧，目的地址是IP层传过来的物理地址，源地址是本机的物理地址，
附上一些控制信息，根据以太网的介质访问规则。将它们传送出去。
主机B收到这个数据帧后，先检查它的目的地址，并和本机的物理地址对比，如符合，则接收，否则丢弃。同样，IP层检查后，将有用的信息撮后交给
ICMP协议，ICMP协议处理后，马上构建一个ICMP应答包，发送给主机A，其过程与主机A发送ICMP请求包给主机B一样。
- 不在同一个网段内
在主机A运行"ping 192.168.0.1"后，开始跟上面一样，到了怎样得到MAC地址时，IP协议通过计算（将IP地址与子网掩码作与（&）运算，如果结果
相同，则在同一个网段内）发现目的IP地址与主机A不在同一网段内，就直接将交由路由处理，也就是将路由的MAC取过来。
取MAC地址的方式与在同一个网段内取MAC地址的方式大致一样：先在ARP缓存表里面取，取不到就广播。后续流程大致一样。
